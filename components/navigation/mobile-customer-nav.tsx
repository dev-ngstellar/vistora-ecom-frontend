      ...
      >
        ...
        <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-900 text-white">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center font-bold text-sm text-white">
              V
            </div>
            <span className="font-bold text-sm tracking-wider uppercase">Vistora</span>
          </div>
        ...
        {isAuthenticated && user ? (
          ...
          <div className="p-4 border-b border-slate-100 grid grid-cols-2 gap-2">
            ...
            <Link
              href="/auth/register"
              onClick={() => setIsOpen(false)}
              className="flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl bg-indigo-600 text-xs font-semibold text-white hover:bg-indigo-700 transition"
            >
          ...
          {customerNavItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <ChevronRight className={`w-3.5 h-3.5 ${isActive ? 'text-indigo-200' : 'text-slate-400'}`} />
              </Link>
            );
          })}
